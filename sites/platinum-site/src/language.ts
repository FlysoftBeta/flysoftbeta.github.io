export const _data: Record<string, Record<string, string>> = {
    "zh-cn": {
        com_unknown: "无法在语言文件中找到此条目的本地化文本。",
        com_name: "Platinum",
        com_author: "Flysoft",
        com_close: "关闭",
        cover_sub_placeholder: "-一款为你而生的浏览器-",
        cover_sub_1_placeholder: "口",
        ...
        notice_content:
            "⚠️下载前提示: Windows 版需要 Windows 7 及以上版本的 Windows 操作系统运行，仅支持 64 位和 32 位操作系统。Linux 版仅支持 64 位操作系统，相对于 Windows 版阉割了一些功能。Microsoft Store 内的 UWP 版仅支持 64 位操作系统，且可能不是最新版的 Platinum，相对于 Windows 版阉割了一些功能。",
    ...
    "zh-tw": {
        ...
        notice_content:
            "⚠️下載前提示: Windows 版需要 Windows 7 及以上版本的 Windows 運行，僅支持 64-bit 和 32-bit 的作業系統。 Linux 版僅支持 64-bit 作業系統，相對於 Windows 版閹割了一些功能。Microsoft Store 內的 UWP 版僅支持 64-bit 作業系統，且可能不是最新版的 Platinum，相對於 Windows 版閹割了一些功能。",
    ...
    "en-us": {
        ...
        notice_content:
            "⚠️Attention: Platinum for Windows requires Windows 7 and above, and it only supports 64-bit and 32-bit Windows. Platinum for Linux only supports 64-bit Linux. Platinum for UWP in the Microsoft Store only supports 64-bit operating systems, and maybe it is not the latest version of Platinum.",
...
export var _name: keyof typeof _data;
...
export function get(msg: string, replace = [], fallback: boolean = true): string {
    ...
    var data: string = _data[_name][msg] as any;
    if (!data && !fallback) return "Error";
